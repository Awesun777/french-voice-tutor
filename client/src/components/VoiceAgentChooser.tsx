 ...
 * Anna's video isn't available yet, so her half renders a styled placeholder;
 * drop `anna.mp4` into client/public/avatars/ and set `video` below to swap it in.
 */
...
function AgentAvatar({ agent, active }: { agent: AgentConfig; active: boolean }) {
  const ringCls = cn(
    "relative h-36 w-36 md:h-56 md:w-56 rounded-full overflow-hidden ring-4 transition-all duration-300",
  ...
  // Placeholder until anna.mp4 is provided.
  return (
    <div className={cn(ringCls, "bg-gradient-to-br", agent.placeholder)}>
...
function VoiceAgentChooser({ onStartReview }: { onStartReview: (dateKey?: string) => void }) {
  ...
  if (started) {
    const agent = AGENTS.find((a) => a.id === started)!;
    return (
      <div className="flex flex-col h-full overflow-hidden">
        <div className="flex-shrink-0 border-b border-border bg-background/80 backdrop-blur-sm px-4 py-2 flex items-center gap-3">
          ...
          <span className="text-[10px] text-muted-foreground">{agent.tag}</span>
        </div>
